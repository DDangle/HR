...
class DepartmentInfoVC: UITableViewController {
    ...
    //테이블 뷰에 들어갈 섹션의 수를 결정하는 메소드, 오버라이드 하지않았을때 기본 반환값 1
    override func numberOfSections(in tableView: UITableView) -> Int {
        return 2
    }
    
    //각 섹션 헤더에 들어갈 뷰를 정의하는 메소드, 섹션 별 타이틀을 뷰 형태로 구성하는 메소드
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            let state = UISegmentedControl(items: ["재직중", "휴식", "퇴사"])
            ...
            state.tag = row.empCd //액션 메소드에서 참조할 수 있도록 사원 코드를 작성
